...
public class BitOggStream : System.IDisposable
{
    ...
    private byte[] page_buffer = new byte[(int)AnonymousEnum.header_bytes + (int)AnonymousEnum.max_segments + (int)AnonymousEnum.segment_size * (int)AnonymousEnum.max_segments];
    ...
    public void flush_bits()
    {
        if (bits_stored != 0)
        {
            ...
            page_buffer[(int)AnonymousEnum.header_bytes + (int)AnonymousEnum.max_segments + payload_bytes] = bit_buffer;
            payload_bytes++;
    ...
    public void flush_page(bool next_continued = false, bool last = false)
    {
        ...
        if (payload_bytes != 0)
        {
            uint segments = (payload_bytes + (uint)AnonymousEnum.segment_size) / (uint)AnonymousEnum.segment_size; // intentionally round up
            ...
            {
                page_buffer[(int)AnonymousEnum.header_bytes + segments + i] = page_buffer[(int)AnonymousEnum.header_bytes + (int)AnonymousEnum.max_segments + i];
            }
